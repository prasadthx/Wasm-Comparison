...
var u={};b&&b.env&&(u=b.env);var x=[];b&&b.args&&(x=b.args);var v=defaultBindings;b&&b.bindings&&(v=b.bindings);this.view=this.memory=void 0;this.bindings=v;this.FD_MAP=new Map([[WASI_STDIN_FILENO,{real:0,filetype:WASI_FILETYPE_CHARACTER_DEVICE,rights:{base:STDIN_DEFAULT_RIGHTS,inheriting:BigIntPolyfill(0)},path:void 0}],[WASI_STDOUT_FILENO,{real:1,filetype:WASI_FILETYPE_CHARACTER_DEVICE,rights:{base:STDOUT_DEFAULT_RIGHTS,inheriting:BigIntPolyfill(0)},path:void 0}],[WASI_STDERR_FILENO,{real:2,filetype:WASI_FILETYPE_CHARACTER_DEVICE,
rights:{base:STDERR_DEFAULT_RIGHTS,inheriting:BigIntPolyfill(0)},path:void 0}]]);var g=this.bindings.fs,n=this.bindings.path;try{for(var B=__values(Object.entries(t)),A=B.next();!A.done;A=B.next()){var G=__read(A.value,2),K=G[0],H=G[1],L=g.openSync(H,g.constants.O_RDONLY),M=__spread(this.FD_MAP.keys()).reverse()[0]+1;this.FD_MAP.set(M,{real:L,filetype:WASI_FILETYPE_DIRECTORY,rights:{base:RIGHTS_DIRECTORY_BASE,inheriting:RIGHTS_DIRECTORY_INHERITING},fakePath:K,path:H})}}catch(a){var I={error:a}}finally{try{A&&
!A.done&&(d=B.return)&&d.call(B)}finally{if(I)throw I.error;}}var C=function(a,e){c.refreshMemory();return Array.from({length:e},function(e,b){b=a+8*b;e=c.view.getUint32(b,!0);b=c.view.getUint32(b+4,!0);return new Uint8Array(c.memory.buffer,e,b)})},k=function(a,e){a=stat(c,a);if(e!==BigIntPolyfill(0)&&(a.rights.base&e)===BigIntPolyfill(0))throw new WASIError(WASI_EPERM);return a},N=v.hrtime(),D=function(a){switch(a){case WASI_CLOCK_MONOTONIC:return v.hrtime();case WASI_CLOCK_REALTIME:return msToNs(Date.now());
case WASI_CLOCK_PROCESS_CPUTIME_ID:case WASI_CLOCK_THREAD_CPUTIME_ID:return v.hrtime()-N;default:return null}};this.wasiImport={args_get:function(a,e){c.refreshMemory();var b=a,d=e;x.forEach(function(a){c.view.setUint32(b,d,!0);b+=4;d+=isomorphicBuffer.from(c.memory.buffer).write(a+"\x00",d)});return WASI_ESUCCESS},args_sizes_get:function(a,e){c.refreshMemory();c.view.setUint32(a,x.length,!0);a=x.reduce(function(a,c){return a+isomorphicBuffer.byteLength(c)+1},0);c.view.setUint32(e,a,!0);return WASI_ESUCCESS},
environ_get:function(a,e){c.refreshMemory();var b=a,d=e;Object.entries(u).forEach(function(a){var e=__read(a,2);a=e[0];e=e[1];c.view.setUint32(b,d,!0);b+=4;d+=isomorphicBuffer.from(c.memory.buffer).write(a+"="+e+"\x00",d)});return WASI_ESUCCESS},environ_sizes_get:function(a,e){c.refreshMemory();var b=Object.entries(u).map(function(a){a=__read(a,2);return a[0]+"="+a[1]+"\x00"}),d=b.reduce(function(a,c){return a+isomorphicBuffer.byteLength(c)},0);c.view.setUint32(a,b.length,!0);c.view.setUint32(e,d,
!0);return WASI_ESUCCESS},clock_res_get:function(a,e){switch(a){case WASI_CLOCK_MONOTONIC:case WASI_CLOCK_PROCESS_CPUTIME_ID:case WASI_CLOCK_THREAD_CPUTIME_ID:var b=BigIntPolyfill(1);break;case WASI_CLOCK_REALTIME:b=BigIntPolyfill(1E3)}c.view.setBigUint64(e,b);return WASI_ESUCCESS},clock_time_get:function(a,e,b){c.refreshMemory();a=D(a);if(null===a)return WASI_EINVAL;c.view.setBigUint64(b,BigIntPolyfill(a),!0);return WASI_ESUCCESS},fd_advise:wrap(function(a,c,b,d){k(a,WASI_RIGHT_FD_ADVISE);return WASI_ENOSYS}),
...
g.ftruncateSync(a.real,Number(c));return WASI_ESUCCESS}),fd_filestat_set_times:wrap(function(a,c,b,d){a=k(a,WASI_RIGHT_FD_FILESTAT_SET_TIMES);var e=g.fstatSync(a.real),h=e.atime;e=e.mtime;var f=nsToMs(D(WASI_CLOCK_REALTIME)),p=WASI_FILESTAT_SET_ATIM|WASI_FILESTAT_SET_ATIM_NOW;if((d&p)===p)return WASI_EINVAL;p=WASI_FILESTAT_SET_MTIM|WASI_FILESTAT_SET_MTIM_NOW;if((d&p)===p)return WASI_EINVAL;(d&WASI_FILESTAT_SET_ATIM)===WASI_FILESTAT_SET_ATIM?h=nsToMs(c):(d&WASI_FILESTAT_SET_ATIM_NOW)===WASI_FILESTAT_SET_ATIM_NOW&&
...
return WASI_ESUCCESS}),path_create_directory:wrap(function(a,e,b){a=k(a,WASI_RIGHT_PATH_CREATE_DIRECTORY);if(!a.path)return WASI_EINVAL;c.refreshMemory();e=isomorphicBuffer.from(c.memory.buffer,e,b).toString();g.mkdirSync(n.resolve(a.path,e));return WASI_ESUCCESS}),path_filestat_get:wrap(function(a,e,b,d,f){a=k(a,WASI_RIGHT_PATH_FILESTAT_GET);if(!a.path)return WASI_EINVAL;c.refreshMemory();b=isomorphicBuffer.from(c.memory.buffer,b,d).toString();b=g.statSync(n.resolve(a.path,b));c.view.setBigUint64(f,
BigIntPolyfill(b.dev),!0);f+=8;c.view.setBigUint64(f,BigIntPolyfill(b.ino),!0);f+=8;c.view.setUint8(f,translateFileAttributes(c,void 0,b).filetype);f+=8;c.view.setBigUint64(f,BigIntPolyfill(b.nlink),!0);f+=8;c.view.setBigUint64(f,BigIntPolyfill(b.size),!0);f+=8;c.view.setBigUint64(f,msToNs(b.atimeMs),!0);f+=8;c.view.setBigUint64(f,msToNs(b.mtimeMs),!0);c.view.setBigUint64(f+8,msToNs(b.ctimeMs),!0);return WASI_ESUCCESS}),path_filestat_set_times:wrap(function(a,b,d,f,m,r,w){a=k(a,WASI_RIGHT_PATH_FILESTAT_SET_TIMES);
if(!a.path)return WASI_EINVAL;c.refreshMemory();var e=g.fstatSync(a.real);b=e.atime;e=e.mtime;var h=nsToMs(D(WASI_CLOCK_REALTIME)),l=WASI_FILESTAT_SET_ATIM|WASI_FILESTAT_SET_ATIM_NOW;if((w&l)===l)return WASI_EINVAL;l=WASI_FILESTAT_SET_MTIM|WASI_FILESTAT_SET_MTIM_NOW;if((w&l)===l)return WASI_EINVAL;(w&WASI_FILESTAT_SET_ATIM)===WASI_FILESTAT_SET_ATIM?b=nsToMs(m):(w&WASI_FILESTAT_SET_ATIM_NOW)===WASI_FILESTAT_SET_ATIM_NOW&&(b=h);(w&WASI_FILESTAT_SET_MTIM)===WASI_FILESTAT_SET_MTIM?e=nsToMs(r):(w&WASI_FILESTAT_SET_MTIM_NOW)===
WASI_FILESTAT_SET_MTIM_NOW&&(e=h);d=isomorphicBuffer.from(c.memory.buffer,d,f).toString();g.utimesSync(n.resolve(a.path,d),new Date(b),new Date(e));return WASI_ESUCCESS}),path_link:wrap(function(a,b,d,f,m,r,w){a=k(a,WASI_RIGHT_PATH_LINK_SOURCE);m=k(m,WASI_RIGHT_PATH_LINK_TARGET);if(!a.path||!m.path)return WASI_EINVAL;c.refreshMemory();d=isomorphicBuffer.from(c.memory.buffer,d,f).toString();r=isomorphicBuffer.from(c.memory.buffer,r,w).toString();g.linkSync(n.resolve(a.path,d),n.resolve(m.path,r));
return WASI_ESUCCESS}),path_open:wrap(function(a,b,d,f,m,r,w,p,z){b=k(a,WASI_RIGHT_PATH_OPEN);r=BigIntPolyfill(r);w=BigIntPolyfill(w);a=(r&(WASI_RIGHT_FD_READ|WASI_RIGHT_FD_READDIR))!==BigIntPolyfill(0);var e=(r&(WASI_RIGHT_FD_DATASYNC|WASI_RIGHT_FD_WRITE|WASI_RIGHT_FD_ALLOCATE|WASI_RIGHT_FD_FILESTAT_SET_SIZE))!==BigIntPolyfill(0);if(e&&a)var h=g.constants.O_RDWR;else a?h=g.constants.O_RDONLY:e&&(h=g.constants.O_WRONLY);a=r|WASI_RIGHT_PATH_OPEN;r|=w;0!==(m&WASI_O_CREAT)&&(h|=g.constants.O_CREAT,a|=
WASI_RIGHT_PATH_CREATE_FILE);0!==(m&WASI_O_DIRECTORY)&&(h|=g.constants.O_DIRECTORY);0!==(m&WASI_O_EXCL)&&(h|=g.constants.O_EXCL);0!==(m&WASI_O_TRUNC)&&(h|=g.constants.O_TRUNC,a|=WASI_RIGHT_PATH_FILESTAT_SET_SIZE);0!==(p&WASI_FDFLAG_APPEND)&&(h|=g.constants.O_APPEND);0!==(p&WASI_FDFLAG_DSYNC)&&(h=g.constants.O_DSYNC?h|g.constants.O_DSYNC:h|g.constants.O_SYNC,r|=WASI_RIGHT_FD_DATASYNC);0!==(p&WASI_FDFLAG_NONBLOCK)&&(h|=g.constants.O_NONBLOCK);0!==(p&WASI_FDFLAG_RSYNC)&&(h=g.constants.O_RSYNC?h|g.constants.O_RSYNC:
h|g.constants.O_SYNC,r|=WASI_RIGHT_FD_SYNC);0!==(p&WASI_FDFLAG_SYNC)&&(h|=g.constants.O_SYNC,r|=WASI_RIGHT_FD_SYNC);e&&0===(h&(g.constants.O_APPEND|g.constants.O_TRUNC))&&(r|=WASI_RIGHT_FD_SEEK);c.refreshMemory();d=isomorphicBuffer.from(c.memory.buffer,d,f).toString();d=n.resolve(b.path,d);if(n.relative(b.path,d).startsWith(".."))return WASI_ENOTCAPABLE;try{var q=g.realpathSync(d);if(n.relative(b.path,q).startsWith(".."))return WASI_ENOTCAPABLE}catch(J){if("ENOENT"===J.code)q=d;else throw J;}h=g.statSync(q).isDirectory()?
g.openSync(q,g.constants.O_RDONLY):g.openSync(q,h);d=__spread(c.FD_MAP.keys()).reverse()[0]+1;c.FD_MAP.set(d,{real:h,filetype:void 0,rights:{base:a,inheriting:r},path:q});stat(c,d);c.view.setUint32(z,d,!0);return WASI_ESUCCESS}),path_readlink:wrap(function(a,b,d,f,m,r){a=k(a,WASI_RIGHT_PATH_READLINK);if(!a.path)return WASI_EINVAL;c.refreshMemory();b=isomorphicBuffer.from(c.memory.buffer,b,d).toString();b=n.resolve(a.path,b);b=g.readlinkSync(b);f=isomorphicBuffer.from(c.memory.buffer).write(b,f,m);
c.view.setUint32(r,f,!0);return WASI_ESUCCESS}),path_remove_directory:wrap(function(a,b,d){a=k(a,WASI_RIGHT_PATH_REMOVE_DIRECTORY);if(!a.path)return WASI_EINVAL;c.refreshMemory();b=isomorphicBuffer.from(c.memory.buffer,b,d).toString();g.rmdirSync(n.resolve(a.path,b));return WASI_ESUCCESS}),path_rename:wrap(function(a,b,d,f,m,r){a=k(a,WASI_RIGHT_PATH_RENAME_SOURCE);f=k(f,WASI_RIGHT_PATH_RENAME_TARGET);if(!a.path||!f.path)return WASI_EINVAL;c.refreshMemory();b=isomorphicBuffer.from(c.memory.buffer,
b,d).toString();m=isomorphicBuffer.from(c.memory.buffer,m,r).toString();g.renameSync(n.resolve(a.path,b),n.resolve(f.path,m));return WASI_ESUCCESS}),path_symlink:wrap(function(a,b,d,f,m){d=k(d,WASI_RIGHT_PATH_SYMLINK);if(!d.path)return WASI_EINVAL;c.refreshMemory();a=isomorphicBuffer.from(c.memory.buffer,a,b).toString();f=isomorphicBuffer.from(c.memory.buffer,f,m).toString();g.symlinkSync(a,n.resolve(d.path,f));return WASI_ESUCCESS}),path_unlink_file:wrap(function(a,b,d){a=k(a,WASI_RIGHT_PATH_UNLINK_FILE);
if(!a.path)return WASI_EINVAL;c.refreshMemory();b=isomorphicBuffer.from(c.memory.buffer,b,d).toString();g.unlinkSync(n.resolve(a.path,b));return WASI_ESUCCESS}),poll_oneoff:function(a,b,d,f){var e=0,g=0;c.refreshMemory();for(var h=0;h<d;h+=1){var k=c.view.getBigUint64(a,!0);a+=8;var n=c.view.getUint8(a);a+=1;switch(n){case WASI_EVENTTYPE_CLOCK:a+=7;c.view.getBigUint64(a,!0);a+=8;var l=c.view.getUint32(a,!0);a+=4;a+=4;n=c.view.getBigUint64(a,!0);a+=8;c.view.getBigUint64(a,!0);a+=8;var q=c.view.getUint16(a,
!0);a+=2;a+=6;var t=1===q;q=WASI_ESUCCESS;l=BigIntPolyfill(D(l));null===l?q=WASI_EINVAL:(n=t?n:l+n,g=n>g?n:g);c.view.setBigUint64(b,k,!0);b+=8;c.view.setUint16(b,q,!0);b+=2;c.view.setUint8(b,WASI_EVENTTYPE_CLOCK);b+=1;b+=5;e+=1;break;case WASI_EVENTTYPE_FD_READ:case WASI_EVENTTYPE_FD_WRITE:a+=3;c.view.getUint32(a,!0);a+=4;c.view.setBigUint64(b,k,!0);b+=8;c.view.setUint16(b,WASI_ENOSYS,!0);b+=2;c.view.setUint8(b,n);b+=1;b+=5;e+=1;break;default:return WASI_EINVAL}}for(c.view.setUint32(f,e,!0);v.hrtime()<
...
function(){this.view&&0!==this.view.buffer.byteLength||(this.view=new DataViewPolyfill(this.memory.buffer))};d.prototype.setMemory=function(b){this.memory=b};d.prototype.start=function(b){b=b.exports;if(null===b||"object"!==typeof b)throw Error("instance.exports must be an Object. Received "+b+".");var d=b.memory;if(!(d instanceof WebAssembly.Memory))throw Error("instance.exports.memory must be a WebAssembly.Memory. Recceived "+d+".");this.setMemory(d);b._start&&b._start()};d.prototype.getImportNamespace=
function(b){var d,c=null;try{for(var t=__values(WebAssembly.Module.imports(b)),u=t.next();!u.done;u=t.next()){var x=u.value;if("function"===x.kind&&x.module.startsWith("wasi_"))if(!c)c=x.module;else if(c!==x.module)throw Error("Multiple namespaces detected.");}}catch(g){var v={error:g}}finally{try{u&&!u.done&&(d=t.return)&&d.call(t)}finally{if(v)throw v.error;}}return c};d.prototype.getImports=function(b){switch(this.getImportNamespace(b)){case "wasi_unstable":return{wasi_unstable:this.wasiImport};
case "wasi_snapshot_preview1":return{wasi_snapshot_preview1:this.wasiImport};default:throw Error("Can't detect a WASI namespace for the WebAssembly Module");}};d.defaultBindings=defaultBindings;return d}(),WASI=WASIDefault;exports.WASI=WASI;exports.WASIError=WASIError;exports.WASIExitError=WASIExitError;exports.WASIKillError=WASIKillError;exports.default=WASIDefault;
